...
//			case 'FieldtypePageTable' :
			...
			case 'FieldtypeFile' :
				...
				foreach($items as $item) {
					...
					$this->dbMigrate->bd($files, 'files for page ' . $page->name);
				}
			$attrib[$name] = $contents;
			$this->dbMigrate->bd($attrib, 'attrib');
			break;
			...
			case 'FieldtypeRepeaterMatrix' :
				...
				foreach($page->$field as $item) {
					$this->dbMigrate->bd($item, 'repeater item');
					...
					foreach($subFields as $subField) {
						$this->dbMigrate->bd($subField, 'subfield of type ' . $subField->type);
						...
						$this->dbMigrate->bd($subData, 'subData');
						$itemData = array_merge_recursive($itemData, $subData);
						$this->dbMigrate->bd([$files, $itemDetails['files']], 'merging files');
					...
					$this->dbMigrate->bd($itemData, 'itemData for ' . $item->name);
					...
					*/
					$this->dbMigrate->bd($itemArray, 'repeater array created');
					$contents[] = $itemArray;
					$this->dbMigrate->bd($contents, 'repeater contents 1');
					$repeaterPages[] = $itemParent . $itemName;
					$this->dbMigrate->bd($repeaterPages, '$repeaterPages in getExportPageData');
				}
				$this->dbMigrate->bd($contents, 'repeater contents 2');
				$attrib[$name] = $contents;
				break;
			case 'FieldtypeTextarea' :
				if(is_object($page->$field) && property_exists($page->$field, 'data')) {
					$this->dbMigrate->bd([$page, $field, $page->$field, $page->$field->data], 'page field data');
					$attrib[$name] = $page->$field->data;
				} else {
					$this->dbMigrate->bd([$page, $field, $page->$field], 'page field');
					$attrib[$name] = $page->$field;
					$this->dbMigrate->bd($this->dbMigrate->findRteImageSources($page), 'rte images');
				}
				break;
			default :
				if(is_object($page->$field) && property_exists($page->$field, 'data')) {
		...
		if($page->id == 20570 || $page->id == 21036) $this->dbMigrate->bd([$page, $field, $attrib], 'page, field, attrib in getFieldData');
	...
	protected function getExportStructureData($k, $key, $item, $object, $excludeAttributes, $newOld, $compareType) {
		$this->dbMigrate->bd($name, 'getExportDataStructure');
		$this->dbMigrate->bd($item['type'], 'type');
		$this->dbMigrate->bd($name, 'name');
		$this->dbMigrate->bd($object, 'object in getExportStructureData');
		if(!$object) {
...
//                throw new WireException('missing object' . $item['name']); // for debugging
		...
		if(!$objectData) {
			$this->dbMigrate->bd($objectData, 'objectData in getExportStructureData');
		...
		$this->dbMigrate->bd($objectData, 'objectdata');
		if($item['type'] == 'fields') {
			...
			if(isset($objectData['type']) && in_array($objectData['type'], ['FieldtypeRepeater', 'FieldtypeRepeaterMatrix', 'FieldtypePage', 'FieldtypePageTable', 'FieldtypeImage', 'FieldtypeFile'])) {
				...
				if($f) {
					if(in_array($objectData['type'], ['FieldtypeRepeater', 'FieldtypeRepeaterMatrix', 'FieldtypePage', 'FieldtypePageTable'])) {
						...
						if($templateId) {
							$this->dbMigrate->bd($templateId, 'template id');
							...
							$this->dbMigrate->bd($objectData['template_name'], 'template names');
						}
				...
				if($this->template == ProcessDbMigrate::MIGRATION_TEMPLATE) {
					...
					if($compareType == 'new' and $newOld == 'new' and isset($templateName)) {
						$this->dbMigrate->bd($templateName, 'templatename');
						foreach($allItems as $other) {
							/* @var $other RepeaterDbMigrateItemPage */
							if($other->isUnpublished) continue;
							$this->dbMigrate->bd($other, 'other - item ' . $i);
							$this->dbMigrate->bd($other->dbMigrateType, 'other type');
							if($i >= $k) break;
							$i++;
		...
		$data[$key] = $objectData;
		$this->dbMigrate->bd($data, 'returning objectdata');
		return ['data' => $data, 'files' => []];
	}
	...
	public function compactArray($data, $keyOnly = null) {
		...
		foreach($data as $entry) {
			if(is_array($entry)) foreach($entry as $type => $line) {
				...
				$this->dbMigrate->bd([$type, $line], 'compact item');
				if(is_array($line)) foreach($line as $action => $item) {
					if(is_array($item)) foreach($item as $name => $values) {
						...
						$this->dbMigrate->bd([$name, $values], 'compact item - name, values');
	...
	public function ___array_compare(array $A, array $B) {
		$this->dbMigrate->bd($A, 'array $A');
		$this->dbMigrate->bd($B, 'array $B');
...
//            return 0;
//        });
//        $this->dbMigrate->bd($R, ' sorted array $R');
	...
	public function pruneImageFields($diffs, $newOld) {
		$this->dbMigrate->bd($diffs, 'Page diffs before unset');
		$this->dbMigrate->bd($this->meta('idMap'), 'idMap');
...
//					$diffsRemain = [];
					foreach($data as $fName => $values) {
						...
						$this->dbMigrate->bd([$fName => $values], "[Field name => Values] in pruneImageFields for $pName");
		...
		$this->dbMigrate->bd($diffs, 'Page diffs after unset');
		return $diffs;
	}
	...
	protected function pruneDetails($diffs, $diffsRemain, $field, $pName, $fName, $values, $newOld, $depth = []) {
		...
		$this->dbMigrate->bd(['pName' => $pName, 'fName' => $fName, 'values' => $values, 'newOld' => $newOld, 'depth' => $depth], 'in prune details');
		...
		if($field and ($field->type == 'FieldtypeImage' or $field->type == 'FieldtypeFile')) {
			...
			$this->dbMigrate->bd(['diffs'=> $diffs, 'values' => $values], 'diffs after unset url');
			...
			if($this->meta['idMap'] and count($values) == 2) {     // we have the different 2 vals and an idMap to fix the image/file links
				...
				$this->dbMigrate->bd($newVals, 'new vals');
				if($newVals and $newVals[0] == $newVals[1]) {
					$this->dbMigrate->bd(['pName' => $pName, 'fName' => $fName, 'depth' => $depth], 'unset ALLKEYS');
	...
	public function del($target, $keyChain) {
		$this->dbMigrate->bd(['target' => $target, 'keyChain' => $keyChain], 'diffs entering deepUnset v2');
		foreach($target as $key => $value) {
			$testKeyChain = $keyChain;
			$testKey = array_shift($testKeyChain);
			if(is_array($value)) {
				$this->dbMigrate->bd(['key' => $key, 'testKey' => $testKey, 'testKeyChain' => $testKeyChain], '$value is array');
				...
				$this->dbMigrate->bd(['testKey' => $testKey, 'testKeyChain' => $testKeyChain], '$value is not array');
				unset($target[$key]);
			}
		}
		$this->dbMigrate->bd(['target' => $target, 'keyChain' => $keyChain], 'diffs returning from deepUnset v2');
		return $target;
	}
	...
	protected function replaceLink($html, $idMapArray, $newOld, $checkFileEquality = false) {
		$this->dbMigrate->bd(['html' => $html, 'idMap' => $idMapArray, 'newOld' => $newOld, 'checkFileEquality' => $checkFileEquality], 'In replaceLink');
		if(strpos($html, '<img') === false and strpos($html, '<a') === false) return $html; //return early if no images or links are embedded in html
		$this->dbMigrate->bd($html, 'old html');
		...
		$this->dbMigrate->bd($targetSiteUrl, 'target site url');
		$sourceSiteUrl = ($this->sourceSiteUrl) ?: '/site/';
		$this->dbMigrate->bd($sourceSiteUrl, 'source site url');
...
//			$re = '/(=\"|' . preg_quote($this->wire()->config->httpHost, '/') . ')\/(?!' . preg_quote($siteSegment, '/') . '\/)(.*)(?=\")/mU';
		...
		foreach($idMapArray as $origId => $destId) {
			...
			if($checkFileEquality) {
				...
				$this->dbMigrate->bd([$destPath, $migPath], '[$destPath, $migPath]');
				$this->dbMigrate->bd([$destFiles, $migFiles], '[$destFiles, $migFiles]');
				$commonFiles = array_intersect($destFiles, $migFiles);
				$foundFiles = [];
				foreach($commonFiles as $commonFile) {
					$this->dbMigrate->bd(basename($commonFile), 'checking file in html');
					if(strpos($html, basename($commonFile)) === false) continue;
					$foundFiles[] = $commonFile;
				}
				$this->dbMigrate->bd($foundFiles, 'files in html');
				foreach($foundFiles as $foundFile) {
					$destFile = $destPath . $foundFile;
					$migFile = $migPath . $foundFile;
					$this->dbMigrate->bd([$destFile, $migFile], 'Test file equality');
					if(filesize($destFile) != filesize($migFile)
						or md5_file($destFile) != md5_file($migFile)
					) return $html;
					$this->dbMigrate->bd([$destFile, $migFile], 'Files are equal');
				}
			}
			$html = preg_replace($re, '${1}' . $destId . '$2', $html);
		}
		$this->dbMigrate->bd($html, 'Returning html from replaceLink');
		return $html;
	}
	...
	public function installMigration($newOld, $dummy = false, $quiet = false) {
		...
		*/
		...
		if(!is_dir($migrationPathNewOld)) {
			$this->dbMigrate->bd($migrationPath, '$migrationPath. Name is ' . $name);
		...
		foreach($dataArray as $repeat) {
			foreach($repeat as $itemType => $itemLine) {
				$this->dbMigrate->bd($itemLine, 'itemline');
				foreach($itemLine as $itemAction => $items) {
					$this->dbMigrate->bd($items, 'items');
		...
		$this->dbMigrate->bd($pagesInstalled, 'pages installed');
		if($this->name != 'dummy-bootstrap' && !$dummy && $this->id) {
			...
			$this->dbMigrate->bd($idMapArray, 'idMapArray');
		...
		$this->dbMigrate->bd($newOld, 'finished install');
	...
	protected function fixRteHtml($pagesInstalled, $idMapArray, $newOld) {
		foreach($pagesInstalled as $page) {
			$this->dbMigrate->bd($page, 'RTE? page');
			foreach($page->getFields() as $field) {
				$this->dbMigrate->bd([$page, $field], 'RTE? field');
				if($field->type == 'FieldtypeTextarea') {
					$this->dbMigrate->bd([$page, $field], 'RTE field Y');
					$this->dbMigrate->bd($page->$field, 'Initial html');
					$html = $page->$field;
					$html = $this->replaceLink($html, $idMapArray, $newOld);
					$this->dbMigrate->bd($html, 'returned html');
					$page->$field = $html;
					$page->of(false);
	...
	protected function installFields($items, $itemType, $quiet = false) {
		...
		// now the page refs
		$this->dbMigrate->bd($pageRefs, 'page refs');
		$newPageRefs = [];
		foreach($pageRefs as $fName => $fData) {
			...
			if(isset($fData['parent_path'])) {
				...
				if($pt && $pt->id) {
					...
					$this->dbMigrate->bd($pPath, 'set parent to id ' . $pt->id);
				} else {
					$error = sprintf(
		...
		$this->dbMigrate->bd($newPageRefs, 'new page refs');
		...
		foreach($names as $name) {
			...
			$this->dbMigrate->bd([$this, $f], '$this, $f: pre-getting export data');
			if($f) {
...
//            }
			...
			if(!$field) {
				...
				$new = false;
				//MDE added
				$this->dbMigrate->bd($fieldData, 'data in processImport before resetting');
				...
				foreach($oldData as $key => $value) {
					if(!isset($fieldData[$key]) and $key != 'id') {
						$field->remove($key);
						$this->dbMigrate->bd($key, 'removed key');
					}
				}
				$field->save();
				$this->dbMigrate->bd($field->getArray(), ' field data in processImport after resetting');
				//MDE end of added
			}
...
//            }
			...
			try {
				$this->dbMigrate->bd($fieldData, 'fieldData');
				...
				$this->dbMigrate->bd($changes, $field->name . ': changes in processimport');
				...
				foreach($changes as $key => $info) {
					if($info['error'] and strpos($key, 'export_options') !== 0) {  // options have been dealt with by fix below, so don't report this error
						$this->dbMigrate->bd(get_class($field->type), 'reporting error');
				...
				$this->dbMigrate->bd($field, 'field before save');
				$field->save();
		...
		$this->dbMigrate->bd($field, 'Field after save');
...
//			$this->dbMigrate->bd([$t, $result], 'template result');
	...
	public function saveItem($item) {
		$this->dbMigrate->bd(debug_backtrace(), ' Backtrace in saveitem');
		$this->dbMigrate->bd($item, '$item in saveItem');
		$fieldgroup = $item->fieldgroup;
		$this->dbMigrate->bd($fieldgroup, '$fieldgroup in saveItem');
		$fieldgroup->save();
		...
		//$this->testContext(); // for debugging
		$savedItem = $item->save();
		$this->dbMigrate->bd($savedItem, 'saved template');
		if(!$item->fieldgroups_id) {
			$this->dbMigrate->bd($item, 'setting fieldgroup');
			$item->setFieldgroup($fieldgroup);
			$savedItem2 = $item->save();
			$this->dbMigrate->bd($savedItem2, 'saved template with new fieldgroup');
		}
		//$this->testContext(); // for debugging
	}
	...
	public function testContext() {
		...
		$f = $f->getContext($t);
		$a = $f->getArray();
		$this->dbMigrate->bd($a, 'Result of testContext');
	}
	...
	protected function installPages($items, $itemType, $newOld, $migrationArray) {
		$items = $this->pruneKeys($items, $itemType);
		$this->dbMigrate->bd($items, 'items in install pages');
		$this->dbMigrate->bd($migrationArray, 'migrationArray in install pages');
		...
		$this->dbMigrate->bd($this);
		$this->sourceSiteUrl = ($migrationArray['sourceSiteUrl']) ??  '';
		$this->dbMigrate->bd($items, 'items after replaceAdminPath');

		$pagesInstalled = [];
		foreach($items as $name => $data) {
			$this->dbMigrate->bd([$name, $data], 'item in install pages');
			if(!$data) continue;
			...
			if(!$this->wire()->sanitizer->path($name)) {
				$this->dbMigrate->bd([$name, $data], 'name, data being passed as not a path');
				continue;
			}
			$this->dbMigrate->bd([$name, $data], 'name, data to install');
			...
			$this->dbMigrate->bd($data['parent'], 'data[parent]');
			$this->dbMigrate->bd($parent, 'PARENT');
			...
			if($p and $p->id) {
				$this->dbMigrate->bd($p, 'page to update');
				$p = $this->updatePage($p, $name, $data, $fields, $repeaters, $newOld);
			} else {
				$this->dbMigrate->bd($name, 'new page');
			...
			$p->of(false);
			$p->save();
			$this->dbMigrate->bd($p, 'saved page at end of install');
			$pagesInstalled[] = $p;
		}
	...
	public function updatePage($p, $name, $data, $fields, $repeaters, $newOld) {
		...
		$this->dbMigrate->bd($fields, 'SAVE complex in update');
		$fields = $this->setAndSaveComplex($fields, $p); // sets and saves 'complex' fields, returning the other fields
		$p->of(false);
		$this->dbMigrate->bd($p, 'SAVE page in update');
		$p->save();
		$fields = $this->setAndSaveFiles($fields, $newOld, $p); // saves files and images, returning other fields
		$this->dbMigrate->bd($fields, 'fields to save');
		$this->dbMigrate->bd($fields, 'SAVE fields in update');
		$p->setAndSave($fields);
		$this->dbMigrate->bd($repeaters, 'SAVE repeaters in update');
	...
	public function newPage($name, $data, $fields, $repeaters, $newOld) {
		...
		$p->status = $data['status'];
		$p->parent = $data['parent'];
		$this->dbMigrate->bd($p, 'saving new page');
		$p->of(false);
		$p->save();
	...
	public function getRepeaters($values) {
		$repeaters = [];
		$this->dbMigrate->bd($values, 'values before removing repeaters');
		foreach($values as $fieldName => $data) {
			$f = $this->wire('fields')->get($fieldName);
			$this->dbMigrate->bd($f, "field for $fieldName");
		...
		$this->dbMigrate->bd($repeaters, 'repeaters');
	...
	public function setAndSaveComplex($fields, $page = null) {
		...
		foreach($fields as $fieldName => $fieldValue) {
			if($fieldValue !== false) {
				$f = $this->wire()->fields->get($fieldName);
				if($f) {
					$this->dbMigrate->bd(['page' => $page->name, 'field' => $f->name, 'field value' => $page->$f, 'field type' => gettype($page->$f)], 'field type in setAndSaveComplex');
					// object-types
					if($f->type == 'FieldtypeStreetAddress' or $f->type == 'FieldtypeSeoMaestro' or $f->type == 'FieldtypeMeasurement') {
						...
						$this->dbMigrate->bd([$fieldName, $fieldValue], 'fieldName, fieldValue of PageTable in setAndSaveComplex');
						foreach($fieldValue['items'] as $item) {
							$this->dbMigrate->bd($item, 'item in PageTable');
						...
						$this->dbMigrate->bd($page, 'page');
						$this->dbMigrate->bd($p, 'p');
						$this->dbMigrate->bd($fieldName, 'fieldName');
						if($page->$fieldName) {
						...
						$page->of(false);
						$page->$fieldName = $a;
						$this->dbMigrate->bd($page->$fieldName, 'page field value');
					} else if($f->type == 'FieldtypeTemplates') {
						$a = [];
						$this->dbMigrate->bd($fieldValue, 'field type templates');
						foreach($fieldValue as $item) {
						...
						$page->of(false);
						$page->$fieldName = $a;
						$this->dbMigrate->bd($page->$fieldName, 'template field value');
					} else {
						continue;
					}
	...
	public function setAndSaveFiles(array $fields, string $newOld, $page = null, $replace = true, $remove = true) {
		...
		foreach($fields as $fieldName => $fieldValue) {
			...
			if($f and ($f->type == 'FieldtypeImage' or $f->type == 'FieldtypeFile')) {
				...
				$this->dbMigrate->bd($existingItems, '$existingItems');
				$this->dbMigrate->bd($fieldValue, 'proposed value');
				$existingItemBasenames = array_filter($existingItems, function($v) {
					return basename($v->url);
				});
				$this->dbMigrate->bd($existingItemBasenames, '$existingItemBasenames');
				$proposedItemBasenames = $fieldValue['items'];
				array_walk($proposedItemBasenames, function(&$v) {
					$v = $v['basename'];
				});
				$this->dbMigrate->bd($proposedItemBasenames, '$proposedItemBasenames');
				...
				$this->dbMigrate->bd(['field' => $fieldValue, 'Venn' => [$notInProposed, $notInExisting, $inBoth]], 'Venn [$notInProposed, $notInExisting, $inBoth]');
				...
				if($remove) foreach($notInProposed as $item) {
					$this->dbMigrate->bd($page->$f, ' Should be page array with item to delete being ' . $item);
					$page->$f->delete($item);
				}
				foreach($fieldValue['items'] as $item) {
					if(array_key_exists($item['basename'], $inBoth) and $replace) {
						$this->dbMigrate->bd($item['basename'], 'deleting file to be replaced');
					...
					$this->dbMigrate->bd($addFile, 'adding file');
...
//                    $page->$f->$pageFile->formatted = $item['formatted']; // NB Removed this as ->formatted does not seem to be consistent and may cause spurious mismatches
					$this->dbMigrate->bd($page->$f->$pageFile, 'Pagefile object');
	...
	public function setAndSaveRepeaters(array $repeaters, $newOld, $page = null, $options = []) {
		...
		foreach($repeaters as $repeaterName => $repeaterData) {
			...
			*/
			$this->dbMigrate->bd([$page, $repeaters], 'page, repeaters:  at start of set and save repeaters');
			$this->dbMigrate->bd($repeaterName, 'repeaterName');
			$this->dbMigrate->bd($repeaterData, 'repeaterData');
			...
			foreach($subPages as $subPage) {
				...
				$this->dbMigrate->bd($subFields, 'subfields');
				$subFieldArray = [];
				foreach($subFields as $subField) {
					$subDetails = $this->getFieldData($subPage, $subField);
					$this->dbMigrate->bd($subDetails, 'subdetails');
...
//				$this->dbMigrate->bd($subPageArray, 'Array from existing subpages of motif_image_component before array_filter');
//				$this->dbMigrate->bd($repeaterData, 'Array of subpages to be set before array_filter');
...
//				$this->dbMigrate->bd($subPageArray, 'Array from existing subpages of motif_image_component after array_filter');
//				$this->dbMigrate->bd($repeaterData, 'Array of subpages to be set after array_filter');
			...
			foreach($subPageArray as $i => $oldSubPage) {    // $i allows us to find the matching existing subpage (in next line)
				...
				foreach($repeaterData as $j => $setSubPage) {
					$this->dbMigrate->bd([$oldSubPage, $setSubPage], 'old and set subpages');

					if($oldSubPage == $setSubPage) {
						unset($repeaterData[$j]);
						$found = true;
						$this->dbMigrate->bd($oldSubPage, 'subpage not changed, resetting sort');
						if($subPage->sort != $j) {
							$subPage->sort = $j;
							$this->dbMigrate->bd($subPage, 'SAVE subpage');
							$subPage->of(false);
				...
				if(!$found) {
					$this->dbMigrate->bd($subPage, 'removing subpage');
...
//					$page->of(false);
					...
					if($repeaterField->type == 'FieldtypeRepeaterMatrix') {
						...
						$this->dbMigrate->bd($typeAttr, 'typeattr');
						$matrixType = $repeaterField->$typeAttr;
						$this->dbMigrate->bd($matrixType, 'matrixtype');
...
//					$newSubPage->save(null, $options);
						...
						$this->dbMigrate->bd($page->fields, 'allowed fields');
...
//				$newSubPage->setAndSave($item, $options);
					...
					$this->dbMigrate->bd($newSubPage, 'SAVE newsubpage');
					...
					$this->dbMigrate->bd($newSubPage, 'SAVE newsubpage2');
					$newSubPage->save(null, $options);
					$this->dbMigrate->bd($newSubPage, 'added new subpage');
					$this->dbMigrate->bd($page, 'saved page after new sub page');
				}
				$this->dbMigrate->bd($page, 'SAVE page2');
		...
		//$page->of(false);
		$page->save(null, $options);
		$this->dbMigrate->bd($page, 'page at end of set and save repeaters');
	}
	...
	protected function removeItems($items, $itemType) {
		...
		$this->dbMigrate->bd($items, 'items for deletion. item type is ' . $itemType);
		...
		$this->dbMigrate->bd($expandedAll, 'expanded items for deletion');
		$objectsAll = [];
		...
		$this->dbMigrate->bd($objectsAll, 'objects for deletion');
		switch($itemType) {
			case 'pages' :
				$pages = $objectsAll;
				foreach($pages as $p) {
					if($p and $p->id) {
						...
						$fields = $p->getFields();
						foreach($fields as $field) {
							$this->dbMigrate->bd($field, 'field to remove');
						...
						$this->dbMigrate->bd($p, '$p before trash');
						...
						$this->dbMigrate->bd([$p->parent->name, $p->name], ' Parent and name');
						//$this->dbMigrate->bd([trim($this->adminPath, '/'), trim(ProcessDbMigrate::MIGRATION_PARENT, '/')], 'comparators');
						if($this->name == 'bootstrap' and $p->parent->name == trim($this->adminPath, '/')
							...
							$this->dbMigrate->bd($p, 'Deleting children too');
							if($p->isLocked()) $p->removeStatus(Page::statusLocked);
							$p->trash(); // trash before deleting in case any hooks need to operate
							$p->delete(true);
						} else {
							$this->dbMigrate->bd($p, 'Only deleting page - will not delete if there are children. (This is ' . $this->name . ')');
							try {
								if($p->isLocked()) $p->removeStatus(Page::statusLocked);
								try {
									$p->trash(); // trash before deleting in case any hooks need to operate
								...
								$this->dbMigrate->bd($p, '$p before delete');
			...
			case 'templates' :
				foreach($objectsAll as $object) {
					$this->dbMigrate->bd($name, 'deleting ' . $itemType);
					if($object) {
						if($object->flags == 8) {
							$this->wire()->session->error(sprintf(
			...
			case 'fields' :
				foreach($objectsAll as $object) {
					$this->dbMigrate->bd($object, 'deleting ' . $itemType);
					$this->dbMigrate->bd($object->flags, 'FLAGS');
					if($object) {
						if($object->flags == 8) {
							$this->wire()->session->error(sprintf(
	...
	protected function setIdMap($pagesInstalled) {
		...
		if(is_array($pagesInstalled)) foreach($pagesInstalled as $page) {
			...
			$this->dbMigrate->bd($page, 'page in getidmap');
			$this->dbMigrate->bd([debug_backtrace(), DEBUG::backtrace()], 'backtrace');
...
//		$html = str_replace($sourceSiteUrl, $this->wire()->config->urls->site, $html);
		...
		$this->dbMigrate->bd($html, 'old html');
		$this->dbMigrate->bd($newHtml, 'new html');
		return $newHtml;
	}
	...
	public function refresh($found = null) {
		$this->dbMigrate->bd('IN REFRESH');
		$this->dbMigrate->bd($this->migrationsPath, '$this->migrationsPath');
		if(!$this->ready) $this->ready();
		...
		if(!$found) {
			if(is_dir($migrationPath)) {
				...
				$this->dbMigrate->bd($found, 'found file');
			}
		}
		...
		$this->dbMigrate->bd($sourceDb, 'sourceDb');
		if($sourceDb) {
			if($this->dbName
		...
		$this->dbMigrate->bd($this->meta('filesHash'), 'fileshash');
		if(
			($this->meta('filesHash') && $this->meta('filesHash') == $filesHash) &&
			($this->meta('hostDb') && $this->meta('hostDb') == $dbName)
		) {
			$this->dbMigrate->bd($this, 'skipping refresh as no changes');
			return true;
		} else {
		...
		*/
		...
		$this->dbMigrate->bd($this->meta('installable'), 'installable?');
		...
		*/
		$this->dbMigrate->bd($fileContents, 'already found file contents');
		...
		foreach($fileContents as $type => $content) {
			$this->dbMigrate->bd($content, 'content item');
			if(!is_array($content)) continue;
			foreach($content as $line) {
				foreach($line as $pathName => $values) {
					$this->dbMigrate->bd($values, 'values');
					...
					if($this->name != 'bootstrap') {
							...
							foreach($oldContents as $oldType => $oldContent) {
								...
								foreach($oldContent as $oldLine) {
									foreach($oldLine as $oldPathName => $oldValues) {
										...
										$this->dbMigrate->bd($fileTestCompare, '$fileTestCompare in refresh');
										$this->dbMigrate->bd($fileCompare, '$fileCompare in refresh');
									}
								}
							}
						}
						...
						$this->dbMigrate->bd([$fileTestCompare, $scopeChange], '[fileTestCompare, scopeChange]');
						...
						if(file_exists($oldFile) and !$fileCompare and !$scopeChange) {
							...
							$this->dbMigrate->bd($infoOnlyValues, 'info only values');
						...
						*/
						if(is_dir($migrationPath . '/old/') and $installedStatus['uninstalled']) {
							 ...
							 */
							$this->dbMigrate->bd(['installedStatus' => $installedStatus, 'fileCompare' => $fileCompare, 'fileTestCompare' => $fileTestCompare, 'scopeChange' => $scopeChange], 'Before renaming Old');
							...
							if(is_dir($migrationPath . '/archive/')) {
								...
								$this->dbMigrate->bd($migrationPath . '/archive/' . 'old-' . $timeStamp . '/', 'RENAMED FILE');
							}
						}
					}
					$this->dbMigrate->bd($values, ' in page refresh with $values');
					...
					$this->dbMigrate->bd($p, 'p after save');
				}
			}
		}
		return true;
	...
	protected function setMigrationPageValues($p, $values) {
		...
		$this->dbMigrate->bd($r, 'return from getrepeaters');
		$repeaters = $r['repeaters'];
		$values = $r['values'];
		...
		if($p and $p->id and $p->meta() and $p->meta('installable')) {
			...
			$this->dbMigrate->bd([$p, $values], 'page, values');
	...
	public function filesHash($path = null) {
		...
		$this->dbMigrate->bd($path, 'fileshash path');
		$fileArray = $this->wire()->files->find($path . $this->name . '/');
		$this->dbMigrate->bd($fileArray, 'fileArray');
		$hashString = '';
	...
	protected function checkOverlaps($itemList) {
		...
		$intersection = [];
		$intersectionOld = [];
		$this->dbMigrate->bd($itemNames, ' Names for this');
	...
	public function extractElements($itemList, $element) {
		$elements = [];
		$this->dbMigrate->bd($itemList, ' extract ' . $element);
	...
	protected function validateValues($itemList) {
		$errors = [];
		$this->dbMigrate->bd($itemList, 'item list in validate');
		foreach($itemList as $item)
			if($item['type'] == 'pages') {
	...
	public function dependencySort() {
		...
		$this->dbMigrate->bd($items, 'items in dependency sort');
		$items->unique();
		$this->dbMigrate->bd($items, 'unique items in dependency sort');
		$matrix = $this->createDependencyMatrix($items); // also sets temporary field 'mysort' to each item in items
		$this->dbMigrate->bd($items, 'items in dependency sort after creating matrix');
		$this->dbMigrate->bd($matrix, 'matrix before topological sort');
		$sorted = $this->topologicalSort($matrix, $items);
		$this->dbMigrate->bd($sorted, 'sorted');
		foreach($sorted as $elem) {
			$this->dbMigrate->bd($elem, 'elem');
		}
		$i = 0;
		while(!$sorted->isEmpty()) {
			$itemNumber = $sorted->dequeue();
			$this->dbMigrate->bd($itemNumber, 'itemNumber');
	...
	public function getDependencies($migrationItem, $item = null) {
		$this->dbMigrate->bd(['item' => $migrationItem, 'name' => $migrationItem->dbMigrateName, 'sourceData' => $migrationItem->meta('sourceData')], 'item in getDependencies');
		$sourceData = $migrationItem->meta('sourceData');
		//$items = $this->getFormatted('dbMigrateItem');
		$items = $this->dbMigrateItem->find("status=1");
		$this->dbMigrate->bd($items, 'items in getDependencies');
...
//						$templateItem = ($templateItem) ? $templateItem->first() : null;
						$this->dbMigrate->bd($templateItem, 'template item in getDependencies 1');
					}
					if(!$templateItem) {
						...
						$this->dbMigrate->bd($field->template_id, 'template_id');
						if($templates) {
...
//						$templateItem = ($templateItem) ? $templateItem->first()->id : null;
						$this->dbMigrate->bd($templateItem, 'template item in getDependencies 2');
					}
			...
			case 3: // page
				...
				$this->dbMigrate->bd($pagePath, 'pagepath');
				$page = ($item) ?: $this->wire()->pages->get("path={$pagePath}, include=all");
				$this->dbMigrate->bd($page, 'page in getdependencies for page');
				if($sourceData && isset($sourceData['parent_id'])) {
					$parentItem = $this->findMigrationItemsByObjectId('page', [$sourceData['parent_id']]);
//					$parentItem = ($parentItem) ? $parentItem->first() : null;
				}
...
//					$templateItem = ($templateItem) ? $templateItem->first() : null;
				...
				if($sourceData && isset($sourceData['pageRefs'])) {
					...
					$this->dbMigrate->bd($pageArray, 'page array A');
				}
				if(!$pageArray && $page) {
					$fields = $page->getFields();
					$this->dbMigrate->bd($fields, 'fields in getdependencies for page');
					foreach($fields as $field) {
						if(in_array($field->type, ['FieldtypePage', 'FieldtypePageTable'])) {
							$pageRefs = $page->$field;
							$this->dbMigrate->bd($pageRefs, 'pageRefs');
				...
				if($sourceData && isset($sourceData['rteLinks'])) {
					$this->dbMigrate->bd($sourceData['rteLinks'], 'sourcedata - rtelinks');
				...
				if(!$pageArray2 && $page) {
					...
					foreach($pageRefs2 as $pageRef) {
						$this->dbMigrate->bd($pageRef, ' page ref in rte dependencies');
						$pageObject = ($pageRef) ? $this->wire->pages->get("id=$pageRef") : null;
						$this->dbMigrate->bd($pageObject, ' page object in rte dependencies');
						if($pageObject) {
							$pageItem = $items->get("dbMigrateType=3, dbMigrateName={$pageObject->name}");
							if($pageItem) {
								$this->dbMigrate->bd($pageItem, ' page item in rte dependencies');
								// NB this only records dependencies if the other page is in a migration item. Should it be restrictive like this?
...
//				}
				...
				$this->dbMigrate->bd(['template_item' => [$templateItem], 'parent_item' => [$parentItem], 'pageRefs' => $pageArray, 'rteLinks' => $pageArray2], 'return from getDependencies');
	...
	public function findMigrationItemsByObjectId($objectType, $idArray) {
		...
		foreach($items as $item) {
			...
			if($sourceData && isset($sourceData['id']) && isset($sourceData['source']) && $sourceData['source'] == $objectType
				...
				foreach($flatArray as $objectId) {
					...
					$this->dbMigrate->bd($object, 'object in findMigrationItemsByObjectId');
					$name = ($types == 'pages') ? 'path' : 'name';
					$this->dbMigrate->bd([$item->dbMigrateType->value, $item->dbMigrateName], 'item type and name');
		...
		$this->dbMigrate->bd(['type' => $objectType, 'idArray' => $idArray, 'itemArray' => $itemArray], 'findMigrationItemsByObjectId');
		return $itemArray;
	}
	...
	public function createDependencyMatrix(&$items) {
		...
		$this->dbMigrate->bd($items, 'items before sort');
		$items->sort('mysort');
		$this->dbMigrate->bd($items, 'items after sort');
		$size = $items->count();
		// Create a zero-filled matrix [size x size]
		$matrix = array_fill(0, $size, array_fill(0, $size, 0));
		$this->dbMigrate->bd($matrix, 'matrix zeroed');
		$i = 0;
		foreach($items as $item) {
			if($item->isUnpublished) continue;
			$this->dbMigrate->bd([$item->dbMigrateType->value, $item->dbMigrateName], 'item type & name');
			$sourceData = $item->meta('sourceData');
			$this->dbMigrate->bd($sourceData, 'sourceData');
			switch($item->dbMigrateType->id) {
					...
					foreach(['template_item', 'parent_item', 'access_roles'] as $dependencyType) {
						if(isset($dependencies[$dependencyType])) foreach($dependencies[$dependencyType] as $dependency) {
							if($dependency && is_int($dependency)) {
								$this->dbMigrate->bd($dependency, "setting $dependencyType item for field");
				...
				case 2 : // Template
					$dependencies = $this->getDependencies($item);
					$this->dbMigrate->bd([$item->dbMigrateName, $dependencies], 'template item and dependencies');
					foreach(['fields', 'childTemplates', 'parentTemplates', 'accessRoles'] as $dependencyType) {
						if(isset($dependencies[$dependencyType])) foreach($dependencies[$dependencyType] as $dependency) {
							if($dependency && is_int($dependency)) {
								$this->dbMigrate->bd($dependency, "setting $dependencyType item for template");
								$this->setDependencyMatrixEntry($matrix, $items, $dependency, $i);
							}
						}
					}
				...
				case 3 : // Page
					$dependencies = $this->getDependencies($item);
					$this->dbMigrate->bd(['migration item name' => $item->dbMigrateName, 'migration item id' => $item->id, 'dependencies' => $dependencies], 'got dependencies for page');
					foreach(['template_item', 'parent_item', 'pageRefs'] as $dependencyType) {
						if(isset($dependencies[$dependencyType]) && $dependencies[$dependencyType]) {
							foreach($dependencies[$dependencyType] as $dependency) {
								if($dependency && is_int($dependency)) {
									$this->dbMigrate->bd($dependency, "setting $dependencyType item for page");
									$this->setDependencyMatrixEntry($matrix, $items, $dependency, $i);
								}
							}
						}
						$this->dbMigrate->bd($matrix, 'matrix for ' . $dependencyType);
					}
					$this->dbMigrate->bd($matrix, 'final matrix for ' . $item->dbMigrateName);
//
//					$templateItem = (isset($dependencies['template_item'])) ? $dependencies['template_item'] : null;
//					if($templateItem && is_int($templateItem)) {
//$this->dbMigrate->bd($templateItem, 'setting template item for page');
...
//$this->dbMigrate->bd($parentItem, 'setting parent item for page');
...
//						if($dependency && is_int($dependency)) {
//$this->dbMigrate->bd($dependency, 'setting pageref item');
//							$this->setDependencyMatrixEntry($matrix, $items, $dependency, $i);
//						}
//					}
					/*
					...
					if(isset($dependencies['rteLinks'])) foreach($dependencies['rteLinks'] as $dependency) {
						if($dependency && is_int($dependency)) {
$this->dbMigrate->bd($dependency, 'setting rteLink item');
							$this->setDependencyMatrixEntry($matrix, $items, $dependency, $i);
						}
					}
					*/
			 ...
			 */
			$matrix[$i][$i] = 0;
			$this->dbMigrate->bd($matrix[$i], 'matrix for item ' . $i);
			$this->dbMigrate->bd($matrix, 'matrix in loop');
			$i++;
		}
		$this->dbMigrate->bd($matrix, 'matrix after loop');
		return $matrix;
	}
	...
	protected function setDependencyMatrixEntry(&$matrix, $items, $relatedItemId, $i) {
		$this->dbMigrate->bd([$matrix, $items, $relatedItemId, $i], 'params to setDependencyMatrixEntry');
		$relatedItem = $items->get("id=$relatedItemId");
		$this->dbMigrate->bd($relatedItem, 'relatedItem');
		...
		$this->dbMigrate->bd($matrix[$j][$i], "dependency of $i on $j");
		if(($j == 2 && $i == 3) or ($j ==  3 && $i == 2) ) $this->dbMigrate->bd(debug_backtrace());
		$this->dbMigrate->bd($matrix, 'matrix in setDependencyMatrixEntry');
	}
	...
	public function topologicalSort(array $matrix, $items, $try = 0): \SplQueue {
		...
		$this->dbMigrate->bd([$order, $size], 'order, size');
		if($order->count() != $size) {// cycle detected
			if($try) {
				$this->dbMigrate->bd($origMatrix, 'Matrix after failing at second attempt');
				return new \SplQueue;
			} // already tried once
			$this->dbMigrate->bd($origMatrix, 'matrix before findcycles');
			$cycles = $this->findCycles($origMatrix, $items);
			$cycleMessages = $cycles['msg'];
			$negativeMatrix = $cycles['negativeMatrix'];
			$this->dbMigrate->bd($negativeMatrix, 'negativeMatrix');
			$this->dbMigrate->bd($cycleMessages, 'cycles');
			...
			$this->dbMigrate->bd($matrix, 'matrix after removing cycles');
			$order = $this->topologicalSort($matrix, $items, 1);
		}
		$this->dbMigrate->bd($order, 'order');
		return $order;
	}
	...
	****** Find the cycles for error msg *******/
	...
	protected function findCycles($matrix, $items) {
		$size = count($matrix);
		$this->dbMigrate->bd($matrix, 'matrix in findCycle');
		...
		foreach($matrix as $vertex => $edgeArray) {
			if($visited[$vertex] == 'not_visited') {
				...
				$this->dbMigrate->bd($stack, 'stack 1');
				$this->processDFSTree($matrix, $stack, $visited, $detectedCycles);
			}
		}
		$this->dbMigrate->bd($detectedCycles, 'detectedCycles in findCycle');
		$msg = [];
		...
		foreach($detectedCycles as $cycle) {
			...
			$this->dbMigrate->bd($cycle, 'cycle in findCycle');
			$negativeMatrix[end($cycle)][reset($cycle)] = 1;
			$this->dbMigrate->bd($negativeMatrix, 'negativeMatrix in findCycle');
			$textCycle = [];
	...
	protected function processDFSTree($matrix, &$stack, &$visited, &$detectedCycles) {
		$stackTop = end($stack);
		$this->dbMigrate->bd([$stackTop, $matrix[$stackTop]], 'stacktop, matrix[stacktop]');
		if($stackTop !== false) {
			foreach($matrix[$stackTop] as $vertex => $edge) {
				if($edge) {
					$this->dbMigrate->bd('got edge');
					$this->dbMigrate->bd([$vertex, $edge], 'vertex, edge');
					if($visited[$vertex] == 'in_stack') {
						$this->printCycle($stack, $vertex, $detectedCycles);
					} else if($visited[$vertex] == 'not_visited') {
						array_push($stack, $vertex);
						$visited[$vertex] = 'in_stack';
						$this->dbMigrate->bd($stack, 'stack 2');
						$this->processDFSTree($matrix, $stack, $visited, $detectedCycles);
						$this->dbMigrate->bd($stack, 'stack 2a');
					}
				}
			}
	...
	protected function printCycle($stack, $vertex, &$detectedCycles) {  // NB $stack is not returned by reference
		...
		array_pop($stack);
		$count = 0;
		$this->dbMigrate->bd([$cycle, $vertex], 'cycle, vertex - check');
		...
		if($newCycle && count(array_unique($cycle)) !== 1) {
			...
			$this->dbMigrate->bd($cycle, 'cycle added');
		}
		$this->dbMigrate->bd($detectedCycles, 'detectedCycles in printCycle');
	}
	...
	protected function beforeSaveThis(HookEvent $event) {
		$p = $event->arguments(0);
		if(!$p or !$p->id) return;
		$this->dbMigrate->bd($event, 'hook event');
		...
		if(!$p->meta('installable') && !$p->meta('locked') && $p->dbMigrateLogChanges < 2) { // 0 is 'sort items', 1 is 'Log changes', 2 is 'manual'
			$p->dependencySort();
			$this->dbMigrate->bd($p, 'sorted migration before save');
			$event->arguments(0, $p); // will be overridden if save not allowed
		}
		$this->dbMigrate->bd([$p, $this, $p->meta('installable'), $this->meta('installable')], 'page $p in hook with $this and meta  for $p  and $this ');
		...
		if($this->meta('installable')) {
			if($this->meta('allowSave')) {
				...
				$this->dbMigrate->bd($this, 'not saving page');
				$this->dbMigrate->bd(debug_backtrace(), 'BACKTRACE');
	...
	protected function beforeTrashThis(HookEvent $event) {
		$this->dbMigrate->bd([$this, $event], '[$this, $event] in before trash');
		$p = $event->arguments(0);
		if(!$p or !$p->id) return;
		$this->dbMigrate->bd($event, 'hook event');
		if($this->id != $p->id) return;  // only want this method to run on the current instance, not all instances of this class
		...
		if(is_dir($migrationPath)) {
			...
			Also check that this migration is not used by any other database.');
				...
				$migrationPath = $this->migrationsPath . $this->name . '/';
				if(is_dir($migrationPath)) {
					$this->dbMigrate->bd($migrationPath, 'Deleting migration files');
	...
	protected function afterFieldsetRender(HookEvent $event) {
		...
		$this->dbMigrate->bd([$fieldset, $attr], 'fieldset, wrapattr');
